...
## 功能
- Telegram 指令：`/start`、`/help`、`/list`、`/remove`（可點選項目刪除）或 `/remove <#|url>`、`/variant <option#>`（多筆待選時可用 `/variant <#> <option#>`）、`/end`
...
## Features
- Telegram commands: `/start`, `/help`, `/list`, `/remove` (tap-to-remove picker) or `/remove <#|url>`, `/variant <option#>` (or `/variant <#> <option#>` when multiple pending), `/end`
